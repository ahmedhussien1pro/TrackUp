...
window.renderPlatformsView = function renderPlatformsView() {
  ...
  // Determine user's sub-track from subtest results
  ...
  // Filter state (stored on window to survive re-renders)
  ...
  const typeLabel = { free: isAr ? 'مجاني' : 'Free', paid: isAr ? 'مدفوع' : 'Paid', freemium: isAr ? 'Freemium' : 'Freemium' };
          ...
          <div style="flex:1;min-width:0;">
            ...
            ${p.promoCode ? `
              <div style="display:flex;align-items:center;gap:.4rem;margin-top:.5rem;">
                ...
                <code class="promo-code-chip" onclick="copyPromo('${p.promoCode}',event)">${p.promoCode} 📋</code>
              </div>
            ` : ''}
          </div>
      ...
      <div class="page-header" data-aos="fade-up">
        <div>
          ...
          <p class="text-muted" style="font-size:.88rem;margin-top:.4rem;max-width:560px;line-height:1.7;">
            ${isAr
              ? 'منصات مجانية ومدفوعة مختارة بعناية بناءً على تخصصك وأسلوبك. أعضاء الباقة يحصلون على كود خصم حصري.'
      ...
      ${topPlatforms.length ? `
        ...
        <div class="surface-panel section-pad" data-aos="fade-up">
          <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.5rem;margin-bottom:1rem;">
            ...
            <span class="mentor-tag" style="font-size:.8rem;">
      ...
      ` : `
        ...
        <div class="surface-panel section-pad" style="text-align:center;" data-aos="fade-up">
          ...
          <p class="text-muted" style="font-size:.88rem;line-height:1.7;max-width:400px;margin:0 auto .9rem;">
            ${isAr
              ? 'بعد ما تكمل اختبار التخصص هتخلصلك 4 منصات هي الأنسب لك بالضبط.'
...
// Helper: get user's sub-track from subtest results
window.getUserSubtrack = function getUserSubtrack() {
  ...
  questions.forEach(q => {
    ...
    if (!opt) return;
    Object.entries(opt.scores).forEach(([t, pts]) => { scores[t] = (scores[t] || 0) + pts; });
  });
