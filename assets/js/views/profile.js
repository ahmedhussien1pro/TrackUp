// ============================================================
// profile.js — Profile view + Goals/Checklist widget (§6.9/UC5)
//            + Onboarding branching (UC2)
// ============================================================
...
window.renderProfileView = function renderProfileView() {
  ...
  const collegeOptions = (window.COLLEGE_OPTIONS || [])
    .map(opt => `<option value="${opt.value}" ${p.college === opt.value ? 'selected' : ''}>${opt.label[lang]}</option>`)
    .join('');
  ...
  ].map(opt => `<option value="${opt.value}" ${p.year === opt.value ? 'selected' : ''}>${opt.label[lang]}</option>`).join('');
  ...
  // ── Goals checklist data (UC5 / §6.9) ──
  const GOALS = [
    { key: 'profileCompleted', icon: 'user-round',    en: 'Complete your profile',       ar: 'أكمل بياناتك',            view: 'profile' },
    { key: 'testCompleted',    icon: 'clipboard-list',en: 'Take the assessment',          ar: 'اجعل الاختبار',              view: 'test' },
    { key: 'resultsViewed',    icon: 'bar-chart-3',   en: 'View your track results',     ar: 'شوف نتائجك',                 view: 'results' },
    { key: 'detailsOpened',    icon: 'layers-3',      en: 'Explore your track',          ar: 'استكشف مسارك',              view: 'track-details' },
    { key: 'roadmapStarted',   icon: 'route',         en: 'Start your roadmap',          ar: 'ابدأ خارطتك',               view: 'roadmap' },
    { key: 'courseStarted',    icon: 'play-circle',   en: 'Begin a course',              ar: 'ابدأ كورس',                   view: 'platforms' },
    { key: 'premiumUnlocked',  icon: 'crown',         en: 'Upgrade to Premium',          ar: 'فعّل بريميوم',               view: 'pricing' },
    { key: 'sessionBooked',    icon: 'calendar-days', en: 'Book a mentor session',       ar: 'احجز جلسة مرشد',            view: 'session-booking' },
  ];

  const doneCount  = GOALS.filter(g => !!m[g.key]).length;
  const goalPct    = Math.round((doneCount / GOALS.length) * 100);
  const trackLabel = (window.TRACKS && state.selectedTrack)
    ? (window.TRACKS[state.selectedTrack]?.title?.[lang] || state.selectedTrack)
    : (isAr ? 'لم يتحدد بعد' : 'Not set yet');

  // roadmap checklist from roadmapProgress (UC5)
  const roadmapSteps = (window.ROADMAP && window.ROADMAP[state.selectedTrack]) || [];
  const roadmapDone  = roadmapSteps.filter(s => !!state.roadmapProgress[`${state.selectedTrack}_${s.step}`]).length;
      ...
      <div class="surface-panel section-pad" data-aos="fade-up">
        <div style="display:flex;align-items:flex-start;gap:1.25rem;flex-wrap:wrap;">
          ...
          <div style="flex:1;min-width:200px;">
            ...
            <div class="text-muted" style="font-size:.87rem;margin-top:.25rem;">
              ${(COLLEGE_OPTIONS||[]).find(c=>c.value===p.college)?.label[lang] || (isAr?'لم يتحدد الكلية':'College not set')}
              ${p.year ? ` &mdash; ${[
                {value:'1',label:{en:'Year 1',ar:'سنة 1'}},{value:'2',label:{en:'Year 2',ar:'سنة 2'}},
                {value:'3',label:{en:'Year 3',ar:'سنة 3'}},{value:'4',label:{en:'Year 4',ar:'سنة 4'}},
                {value:'5',label:{en:'Year 5',ar:'سنة 5'}},{value:'graduate',label:{en:'Graduate',ar:'خريج'}},
        ...
        <div style="display:flex;flex-wrap:wrap;gap:.5rem;margin-top:1.1rem;padding-top:1rem;border-top:1px solid var(--border);">
          ${badges.map(b => {
            ...
            return `<div class="profile-badge ${earned?'profile-badge--earned':'profile-badge--locked'}">
              <i data-lucide="${earned?b.icon:'lock'}" style="width:13px;height:13px;flex-shrink:0;color:${earned?'var(--accent)':'var(--text-muted)'};"></i>
              <span>${isAr?b.ar:b.en}</span></div>`;
          }).join('')}
        </div>
      </div>

      <!-- ── §6.9 / UC5 — Goals + Roadmap Checklist widget ── -->
      <div class="surface-panel section-pad" data-aos="fade-up">

        <!-- header row -->
        <div style="display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap;margin-bottom:1.1rem;">
          <div>
            <div class="eyebrow" style="margin-bottom:.3rem;">${isAr?'أهداف الرحلة':'Journey Goals'}</div>
            <div style="font-size:.82rem;color:var(--text-muted);">
              ${doneCount}/${GOALS.length} ${isAr?'مكتمل':'completed'}
            </div>
          </div>
          <!-- circular progress -->
          <div style="position:relative;width:52px;height:52px;flex-shrink:0;">
            <svg width="52" height="52" viewBox="0 0 52 52" style="transform:rotate(-90deg);">
              <circle cx="26" cy="26" r="20" fill="none" stroke="var(--border)" stroke-width="3.5"/>
              <circle cx="26" cy="26" r="20" fill="none" stroke="var(--accent)" stroke-width="3.5"
                stroke-dasharray="${Math.round((goalPct/100)*2*Math.PI*20)} ${Math.round(2*Math.PI*20)}"
                stroke-linecap="round"/>
            </svg>
            <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;
              font-size:.7rem;font-weight:800;color:var(--accent);">${goalPct}%</div>
          </div>
        </div>

        <!-- overall progress bar -->
        <div style="background:var(--border);border-radius:99px;height:6px;overflow:hidden;margin-bottom:1.1rem;">
          <div style="height:100%;width:${goalPct}%;background:var(--accent);border-radius:99px;transition:width .5s ease;"></div>
        </div>
        ...
        <div style="display:grid;gap:.45rem;">
          ${GOALS.map((g, i) => {
            ...
            return `
              <div
                onclick="${done ? '' : `navigateTo('${g.view}')`}"
                style="display:flex;align-items:center;gap:.75rem;
                  padding:.55rem .7rem;border-radius:10px;
                  background:${done?'var(--surface-2)':'var(--surface-2)'};
                  border:1px solid ${done?'rgba(22,163,74,.25)':'var(--border)'};
                  cursor:${done?'default':'pointer'};
                  transition:border-color .15s,background .15s;
                  ${done?'':''}opacity:${done?'1':'.92'};"
                ${done?'':` onmouseover="this.style.borderColor='var(--accent)';" onmouseout="this.style.borderColor='var(--border)';"`}>

                <!-- checkbox -->
                <div style="width:1.2rem;height:1.2rem;border-radius:50%;flex-shrink:0;
                  ...
                  ${done?`<i data-lucide="check" style="width:.65rem;height:.65rem;color:#fff;"></i>`:''}
                </div>

                <i data-lucide="${g.icon}" style="width:.9rem;height:.9rem;flex-shrink:0;
                  color:${done?'#16a34a':'var(--text-muted)'};transition:color .2s;"></i>

                <span style="flex:1;font-size:.85rem;font-weight:${done?'600':'500'};
                ...
                ${done
                  ? `<i data-lucide="check-circle" style="width:.8rem;height:.8rem;color:#16a34a;flex-shrink:0;"></i>`
                  : `<i data-lucide="chevron-${isAr?'left':'right'}" style="width:.8rem;height:.8rem;color:var(--text-faint);flex-shrink:0;"></i>`
                }
              </div>
            `;
          }).join('')}
          ...
          <div style="margin-top:1.1rem;padding-top:1rem;border-top:1px solid var(--border);">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.7rem;">
              <div style="font-size:.8rem;font-weight:700;color:var(--text);">
                <i data-lucide="route" style="width:.8rem;height:.8rem;vertical-align:-.1em;"></i>
                ${isAr?`خارطة ${window.TRACKS?.[state.selectedTrack]?.title?.[lang]||''}`:` ${window.TRACKS?.[state.selectedTrack]?.title?.[lang]||''} Roadmap`}
              </div>
              <span style="font-size:.75rem;color:var(--text-muted);">${roadmapDone}/${roadmapSteps.length}</span>
            </div>
            <div style="display:grid;gap:.35rem;">
              ${roadmapSteps.slice(0,5).map(s => {
                const done = !!state.roadmapProgress[`${state.selectedTrack}_${s.step}`];
                return `
                  <div style="display:flex;align-items:center;gap:.6rem;padding:.4rem .5rem;
                    border-radius:8px;background:var(--surface-3);">
                    <div style="width:1rem;height:1rem;border-radius:50%;flex-shrink:0;
                      background:${done?'#16a34a':'transparent'};
                      border:1.5px solid ${done?'#16a34a':'var(--border)'};
                      display:flex;align-items:center;justify-content:center;">
                      ${done?`<i data-lucide="check" style="width:.55rem;height:.55rem;color:#fff;"></i>`:''}
                    </div>
                    <span style="font-size:.8rem;color:${done?'var(--text)':'var(--text-muted)'};
                      font-weight:${done?'600':'400'};
                      text-decoration:${done?'line-through':'none'};flex:1;">
                      ${s.title[lang]}
                    </span>
                    ${done?'':s.step<=2||state.premiumUnlocked
                      ?`<button onclick="navigateTo('roadmap')" style="background:none;border:none;cursor:pointer;
                          font-size:.72rem;color:var(--accent);font-weight:600;padding:0;white-space:nowrap;">
                          ${isAr?'ابدأ':'Start'}
                        </button>`
                      :`<i data-lucide="lock" style="width:.7rem;height:.7rem;color:var(--text-faint);"></i>`
                    }
                  </div>`;
              }).join('')}
              ${roadmapSteps.length > 5 ? `
                <button onclick="navigateTo('roadmap')"
                  style="background:none;border:none;cursor:pointer;font-size:.78rem;
                    color:var(--accent);font-weight:600;padding:.3rem 0;text-align:start;">
                  ${isAr?`شوف كل الخطوات (${roadmapSteps.length})`:`View all steps (${roadmapSteps.length})`}
      ...
      <div style="width:100%;max-width:520px;" data-aos="fade-up">
        ...
        <div class="surface-panel section-pad">
          <div style="text-align:center;margin-bottom:1.5rem;">
            ...
            <h2 style="font-size:1.2rem;font-weight:800;margin-bottom:.4rem;">${isAr?'هل تعرف مجالك الهندسي؟':'Do you know your engineering field?'}</h2>
            <p class="text-muted" style="font-size:.88rem;line-height:1.7;max-width:380px;margin:0 auto;">
              ${isAr?'لو عارف مجالك هدخل عليه مباشرة. لو متأكدش عندنا اختبار هيساعدك.':"If you already know your field, go straight to it. If you're unsure, take the assessment."}
            </p>
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;margin-bottom:1.1rem;">
            <button onclick="setProfileStep('pick')"
              ...
              <div class="text-muted" style="font-size:.76rem;margin-top:.25rem;">${isAr?'هختار مباشرة':'Jump straight in'}</div>
            </button>
            <button onclick="branchToTest()"
              ...
              <div class="text-muted" style="font-size:.76rem;margin-top:.25rem;">${isAr?'خدني الاختبار':'Take the assessment'}</div>
            </button>
          </div>
          <button onclick="setProfileStep(null)" class="btn btn-ghost" style="width:100%;font-size:.8rem;">
      ...
      <div style="width:100%;max-width:540px;" data-aos="fade-up">
        ...
        <div class="surface-panel section-pad">
          <div style="text-align:center;margin-bottom:1.25rem;">
            <h2 style="font-size:1.15rem;font-weight:800;margin-bottom:.35rem;">${isAr?'اختار مجالك':'Pick your field'}</h2>
            <p class="text-muted" style="font-size:.85rem;">${isAr?'هتدخل على صفحة التراك مباشرة':"You'll go straight to the track overview"}</p>
          </div>
          <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:.65rem;margin-bottom:1rem;">
            ${PICK_TRACKS.map(tr=>`
              <button onclick="pickTrackDirect('${tr.id}')"
          ...
          <button onclick="setProfileStep('ask')" class="btn btn-ghost" style="width:100%;font-size:.8rem;">
            <i data-lucide="arrow-left" style="width:.8rem;height:.8rem;"></i>
            ${isAr?'رجوع':'Back'}
          </button>
        </div>
...
window.pickTrackDirect = function pickTrackDirect(trackId) {
  ...
  if (!state.rankedTracks.length) {
    state.rankedTracks = [{ id: trackId, percent: 100, tags: [] }];
  }
  updateProgress('detailsOpened', true);
  showToast((state.language==='ar'?'مجالك: ':'Field: ') + trackId, '#2563eb');
