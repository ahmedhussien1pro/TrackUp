window.renderRoadmapView = function renderRoadmapView() {
  ...
  // Header CTA: if premium active, show session booking instead of upgrade
    ...
    <section class="surface-panel section-pad">
      <div class="page-header" data-aos="fade-up">
        <div>
          ...
          <p class="text-muted" style="margin-top:.8rem;">${isPremium ? (lang === 'ar' ? 'جميع الخطوات مفتوحة.' : 'All steps unlocked.') : t('roadmapPreview')}</p>
        </div>
        ${headerCTA}
      </div>
          ...
          return `
            <div class="step-card ${locked ? 'locked' : ''}" data-aos="fade-up" data-aos-delay="${idx * 70}">
              <div class="page-header">
                <div>
                  <div class="eyebrow">${lang === 'ar' ? `الخطوة ${step.step}` : `Step ${step.step}`}</div>
